# -*- coding: utf-8 -*-
"""
Created on Fri Oct 25 11:05:44 2019
...
# To dos:
#   1. Remove html codes // done
#   2. Fix unicode // Done
#   3. pdf?
#   4. Annex long lists, as option 
#   5. Attachments: Image, Choice list
#   6. Fieldname add, as option // Done
#   7. Split long choice list in columns // Done
#   8. Fix constraint text: regex and more
#   9. If search in appearance, add a sentance //Done
...
layout = [  [sg.Text('Please enter the below information:', font='Calibri 12', text_color='green')],
            [sg.Text('XLSForm'), sg.Input(), sg.FileBrowse()],
            [sg.Text('Word file'), sg.Input(), sg.SaveAs()],
            [sg.Text('Language'), sg.InputText(), sg.Text('leave empty if only one label', text_color='grey')],
            [sg.Text('Form title'), sg.InputText(), sg.Text('leave empty to pull from form title', text_color='grey')],
            [sg.Text('Check the below items to include:', font='Calibri 12', text_color='green')],
            [sg.Checkbox('Calculate fields', default=False)],
            [sg.Checkbox('Relevances', default=True)],
            [sg.Checkbox('Constraints', default=True)],
            [sg.Checkbox('Field names', default=True)],
            [sg.Checkbox('Make tables', default=True)],
            [sg.Checkbox('Remove HTML tags', default=True)],
            [sg.Button('Convert'), sg.Button('Cancel')] ]

# Create the Window
window = sg.Window('Enter information', layout, default_element_size=(40, 1), grab_anywhere=False, icon='ipa.ico')


# Event Loop to process "events" and get the "values" of the inputs
#while True:
event, values = window.read()
if event in (None, 'Convert'):   # if user closes window or clicks cancel
    window.close()
    #Specify name of document to import in quotes.
    excelname=values[0]
    #Specify name to save document as in quotes.
    wordname=values[1]
    docif = re.findall('.docx', wordname)

    if docif==[]:
        wordname = wordname + '.docx'
    ...
    #Show field names with question.
    if values[7]==True:
        fieldname=1
    else:
        fieldname=0
...
for l in string.ascii_uppercase:
    ...
    #Enter column titles that are unicode into dictionary.
        ...
        #survcoldict[unicodedata.normalize('NFKD', survey[l+'1'].value).encode('ascii','ignore').decode('utf8')]=l
        survcoldict[survey[l+'1'].value]=l
    ...
    if isinstance(choices[l+'1'].value, str):
        ...
        #chcoldict[unicodedata.normalize('NFKD', choices[l+'1'].value).encode('ascii','ignore').decode('utf8')]=l
        chcoldict[str(choices[l+'1'].value)]=l
    ...
    if survey[l+'1'].value=='label:'+language:
        survcoldict['label']=l
    elif survey[l+'1'].value=='label::'+language:
        survcoldict['label']=l
    if choices[l+'1'].value=='label:'+language:
        chcoldict['label']=l
    elif choices[l+'1'].value=='label::'+language:
        chcoldict['label']=l
    if survey[l+'1'].value=='hint:'+language:
        survcoldict['hint']=l
    elif survey[l+'1'].value=='hint::'+language:
        survcoldict['hint']=l
    if choices[l+'1'].value=='hint:'+language:
        chcoldict['hint']=l
    elif choices[l+'1'].value=='hint:'+language:
        chcoldict['hint']=l
    


#Raise an error if there is no language entered but there should be.
if "label" not in list(survcoldict.keys()):
...
def QuestionState(query, tip, sort, chart, colnum, tableyesno):
    ...
    if tableyesno==0:
        ...
        #qp.paragraph_format.alignment = WD_ALIGN_PARAGRAPH.JUSTIFY_LOW
        if tip is not None:
...
def OptionList(paths, category):
    ...
    try:
        chcoldict['value'] = chcoldict['name']
    ...
    for y in range(2, choices.max_row+1):
        ...
        if not isinstance(label, str):
            label=str(label) #This needs to be fixed to allow unicode
        #Print name and label in row 
        if list_name==paths:
            op=document.add_paragraph(str(name)+' - '+label)
            op.paragraph_format.space_after = Pt(0) 
    
    next_section = document.add_section(WD_SECTION.CONTINUOUS)
    sectPr = next_section._sectPr         
    cols.set(qn('w:num'),'1')
    ope=document.add_paragraph('')
...
def TableTime(repeatgroup):
    ...
    tabletime=0 #change to 0
    #Go row by row through the survey spreadsheet.
    for x in range(1, survey.max_row+1):
        ...
        if survey[survcoldict['type']+str(x)].value=='end repeat' and survey[survcoldict['name']+str(x)].value==repeatgroup:
            return tabletime
...
def ReplaceRefs(phrase, mode):
    ...
    #
    tempphrase=phrase
    if isinstance(tempphrase, str) or isinstance(tempphrase, str):
...
def TranslateCalc(exp, variety):
    ...
    newexp=newexp.replace('index()', 'serial number ')
...
#        newexp=newexp[0].upper() + newexp[1:]
        newexp = re.sub(r'"[^"]*" [0-9]{3}', lambda x: x.groups()[0].upper(), newexp, 1)
...
def Program(a, b, roundnum, tableyesno=0, repeat=0, repeatcount=0):
    ...
    for x in range(a, b+1):
        ...
        if survey[survcoldict['type']+str(x)].value!=None:
            typ=unicodedata.normalize('NFKD', survey[survcoldict['type']+str(x)].value).encode('ascii', 'ignore').decode('utf8')
        if typ=='':
        ...
        if calculates==1:
            programmed=typ in ['text', 'integer',  'decimal', 'geopoint', 'note', 'begin group', 'end group', 'begin repeat', 'end repeat', 'calculate', 'calculate_here'] or typ.partition(' ')[0] in ['select_one', 'select_multiple']
        else:
            programmed=typ in ['text', 'integer',  'decimal', 'geopoint', 'note', 'begin group', 'end group', 'begin repeat', 'end repeat'] or typ.partition(' ')[0] in ['select_one', 'select_multiple']
        #Add the current question number to the dictionary qnumbers.
        if survey[survcoldict['name']+str(x)].value not in list(qnumbers.keys()):
        ...
        # Add the preloaded file name if searched for options
        app = str(survey[survcoldict['appearance']+str(x)].value)
        search = re.findall('search', app)
    
        if fieldname==1:
            if search!=[]:
                attach = app[app.find("(")+1:app.find(",")]    
                question='(' + str(survey[survcoldict['name']+str(x)].value) +') ' + str(survey[survcoldict['label']+str(x)].value) + ' [The choice list comes from preloaded file ' + str(attach) +']'
            else:
               question='(' + str(survey[survcoldict['name']+str(x)].value) +') ' + str(survey[survcoldict['label']+str(x)].value) 
        else:
            if search!=[]:
                attach = app[app.find("(")+1:app.find(",")]    
                question=survey[survcoldict['label']+str(x)].value + ' [The choice list comes from preloaded file ' + str(attach) +']'
            else:
        ...
        #if question!=None:
         #   question=unicodedata.normalize('NFKD', question).encode('ascii', 'ignore').decode('utf8')
        ...
        if (typ=='calculate' or typ=='calculate_here') and question!=None:
            if fieldname==1:
                callabel ='(' + str(survey[survcoldict['name']+str(x)].value) +') ' +  str(survey[survcoldict['label']+str(x)].value)
            else:
                callabel = str(survey[survcoldict['label']+str(x)].value)
            question=ReplaceRefs(callabel, 'A') + ': ' + 'Calculated -' + TranslateCalc(ReplaceRefs(question, 'A'), 'calculation')
        ...
        if isinstance(hint, str):
            #hint=unicodedata.normalize('NFKD', hint).encode('ascii', 'ignore').decode('utf8')
            hint=str(hint)
            hint=hint+'.'
        ...
        if isinstance(constraint, str):
            unicodedata.normalize('NFKD', constraint).encode('ascii', 'ignore').decode('utf8')
        ...
        if isinstance(relevance, str):
            unicodedata.normalize('NFKD', relevance).encode('ascii', 'ignore').decode('utf8')
        ...
        if typ=='begin repeat':
            j = x -1
#            if survey[survcoldict['type']+str(j)].value=='end repeat':
#                section = document.sections[-1] 
#            else:
            ...
            if repeat % 2==0 and suppress==0:
                if isinstance(relevance, str) and relevances==1:
                    question=question+'  Only ask if '+relevance
            ...
            if tablesinclude==1 and suppress==0:
                #tableyesno=TableTime(unicodedata.normalize('NFKD', survey[survcoldict['name']+str(x)].value).encode('ascii', 'ignore').decode('utf8'))
                tableyesno=TableTime(survey[survcoldict['name']+str(x)].value)
        ...
        if typ=='end repeat':
            ...
            if tableyesno==1:
                ...
                k = x+1
#                if survey[survcoldict['type']+str(k)].value!='begin repeat':
                    # Make next section portrait
                new_section = document.add_section(WD_SECTION.CONTINUOUS)
        ...
        if tableyesno==0:
            ...
            if typ=='begin group':
                if isinstance(relevance, str) and relevances==1:
                    relevance='  Only ask if '+relevance
                    document.add_heading(question+relevance, 1)
                else:
                    document.add_heading(question, 1)
            ...
            if typ=='text' or ((typ=='calculate' or typ=='calculate_here') and calculates==1):
                ...
                ans=document.add_paragraph('___________________________________________')
        ...
        #If we are currently printing a table, print the questions this way:
            ...
            if typ=='begin group':
                ...
                table.cell(0, newcol).text='BEGIN GROUP: '+question+ str(relevance)
        ...
        if repeat % 2==1 and suppress==0 and typ=="begin repeat" and tableyesno==0:            
            ...
            if not isinstance(survey[survcoldict['repeat_count']+str(x)].value, int):
                Program(c, d, ' [will repeat as needed]', 0, 1)
            else:
                for i in range(0, repeatcount):
                    Program(c, d, ': Repeat group'+str(i+1), 0, repeat)
...
sg.Popup('Your file is save here: '+ wordname, icon='ipa.ico', title='Done!') 
path=os.path.dirname(os.path.abspath(wordname))
os.startfile(path)